...
def Query2(postal):
    ...
    Query2 = ''' SELECT COUNT(*), AVG(s.size) FROM Orders O, Customers C, OrderSize s WHERE O.customer_id = C.customer_id
                AND C.customer_postal_code=:postal AND s.oid=O.order_id
            '''
    t = time.process_time()
...
def user_opt(postal):
    ...
    i = ''' CREATE INDEX customer_index ON Customers (customer_postal_code, customer_id);
